...
if __name__ == "__main__":
    ...
    model = Net(120, 160, len(actions))
    opt = optim.RMSprop(params=model.parameters(), lr=lr)
    out_f = open('acc_log.txt', 'w')
    ...
    while True:
        ...
        while not game.is_episode_finished():
            ...
            screen_buf_t = screen_buf_t.permute(2, 0, 1)
        ...
        per_timestep_losses = torch.stack(per_timestep_losses)
        # print(f"per_timestep_losses: {per_timestep_losses}")
        loss = per_timestep_losses.sum()
        loss.backward()
        batch_loss += loss.item()
        batch_reward += game.get_total_reward()
        if (i + 1) % accumulate_episodes == 0:
            b = i // accumulate_episodes
            batch_reward_avg = batch_reward / accumulate_episodes
            batch_loss_avg = batch_loss / accumulate_episodes
            print(f"batch {b} avg batch reward: {batch_reward_avg} avg batch_loss: {batch_loss_avg: .4f}")
            opt.step()
            opt.zero_grad()
            ...
            batch_reward = 0.0
        if i % 10000 == 0:
            torch.save(model, f'/home/krjeo/PycharmProjects/pythonProject/accumulation_models/{i}th_trained_model.pt')
            print(f'Saved model {i}th trained')
        i += 1
